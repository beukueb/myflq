...
When custom loci.csv and alleles.csv are required, one can submit them on [Github](https://github.com/beukueb/myflq) with a pull request (ask a bioinformatician to help if you don't know how). The program will then be rebuild, and your files will be available to select within 24 hours.
...
- For each locus, having the visual indication close by, go over the alleles and validate them by selecting the checkbox, or if they seem of poor quality, deselect them.
...
- A new browser window will open with all the alleles in the profile. Save it locally.  
  (For now it is not possible to save it in your project, so choose a filename that indicates the project/result.) 
